...
/** 分配下一个可用 ID（取现有最大序号 + 1，至少 3 位补零） */
export function nextId(ws: Workspace, kind: EntityKind): string {
  const lists: Record<EntityKind, Array<{ entity: { id: string } }>> = {
    epic: ws.epics,
    feature: ws.features,
    story: ws.stories,
  };
  let max = 0;
  for (const { entity } of lists[kind]) {
    const num = Number(entity.id.split('-')[1]);
    if (Number.isFinite(num) && num > max) max = num;
  }
  const next = max + 1;
  return `${ID_PREFIX[kind]}-${String(next).padStart(3, '0')}`;
}
...
export async function writeEntity(
  ...
  const file = entityFilePath(ws, kind, id);
